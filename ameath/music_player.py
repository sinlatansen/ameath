...
class MusicPlayer:
    ...
    def create_gui(self):
        """创建GUI"""
        self.parent.title("飞行雪绒 - 音乐播放器")
        self.parent.geometry("420x580")
        ...
        x = (screen_w - 420) // 2
        y = (screen_h - 580) // 2
        self.parent.geometry(f"420x580+{x}+{y}")

        main_frame = tk.Frame(self.parent, bg=self.blue_sky, padx=20, pady=15)
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        # 创建标题框架
        title_frame = tk.Frame(main_frame, bg=self.blue_sky)
